...
> per-engine wall analysis (Ostrich 8192 / MJX 2 / SI 512), and what the
...
|---|---|---|---|
| `ostrich_sim.py` | Ostrich | implicit adjoint | `builder.finalize_replicated(num_worlds=N)` |
...
python experiments/4_scalability_box/ostrich_sim.py --num-worlds 64 \
    --save experiments/4_scalability_box/results/ostrich_64.json
...
- Iterates powers of two from 1 up to the configured ceiling per engine
  (Ostrich: 131k, MJX: 64, SI: 32 — past that we expect OOM).
...
|---|---|---|---|
| Ostrich         | ~0.5 s × 5 = 2.5 s    | ~60 s (module compile cached after run 1) | ~30 min for 18 points |
...
Ostrich/SI, JAX's `peak_bytes_in_use` for MJX) and is included for context but
...
```json
{
  "simulator": "Ostrich" | "MJX-grad" | "Semi-Implicit",
  "num_worlds": 64,
  "median_time_ms": 95.2,
  "peak_gpu_mb": 215.0,
