...
def assign_joint_q_qd_obs(
    ...
    q_offset = 0
    if joint_type[tid] == newton.JointType.FREE: #wp.sim.JOINT_FREE:
...
class Environment:
    ...
    custom_dynamics: Callable[
        [newton.Model, newton.State, newton.State, float, newton.Control], None
    ] = None
    ...
    def __init__(
        ...
        builder = newton.ModelBuilder(up_vector=up_vector, gravity=self.gravity)
        ...
        try:
            articulation_builder = newton.ModelBuilder(up_vector=up_vector, gravity=self.gravity)
        ...
        if self.integrator_type == IntegratorType.EULER:
            ...
            self.integrator = newton.solvers.SolverSemiImplicit(**self.euler_settings)#wp.sim.SemiImplicitIntegrator(**self.euler_settings)
        elif self.integrator_type == IntegratorType.XPBD:
            self.sim_substeps = self.sim_substeps_xpbd
            self.integrator = newton.solvers.SolverXPBD(**self.xpbd_settings)#wp.sim.XPBDIntegrator(**self.xpbd_settings)
            ...
            self.integrator = newton.solvers.SolverFeatherstone(self.model, **self.featherstone_settings) #wp.sim.FeatherstoneIntegrator(self.model, **self.featherstone_settings)
