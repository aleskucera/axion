...
class Simulator(AbstractSimulator):
    ...
    def build_model(self) -> newton.Model:
        ...
        # Capsule Params
        # Box was length 0.6 (2*0.3).
        # Capsule radius 0.1. Length = 0.6.
        # Length = 2*(hh + r) => 0.6 = 2*(hh + 0.1) => 0.3 = hh + 0.1 => hh = 0.2
        capsule_radius = 0.1
        capsule_hh = 0.2
        # Rotate capsule from Z to X (90 deg around Y)
        q_cap_rot = wp.quat_from_axis_angle(wp.vec3(0.0, 1.0, 0.0), -wp.pi / 2.0)
        X_cap_local = wp.transform(wp.vec3(0.0), q_cap_rot)

        # 3. Create Chain Articulation
        ...
        normal = np.array([-tan_2d[1], tan_2d[0], 0.0])
        binormal = np.array([0.0, 0.0, 1.0])
        R_local = np.column_stack((tangent, normal, binormal))
        ...
        # Use Capsule instead of Box
        self.builder.add_shape_capsule(
            head,
            radius=capsule_radius,
            half_height=capsule_hh,
            xform=X_cap_local,
        ...
        for i in range(1, num_links):
            ...
            ball_joint = self.builder.add_joint(
                ...
                custom_attributes={"joint_compliance": 1e-6},
            )
...
def track_chain_capsule_example(cfg: DictConfig):
