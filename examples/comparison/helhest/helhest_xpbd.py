"""Helhest trajectory optimization using Newton XPBD solver (BPTT).

Comparable to examples/comparison/helhest/helhest_axion.py.

Uses the same XPBD contact model and identical DT=5e-2 / T=60 timesteps as
helhest_axion.py, so the only difference is the gradient method:
  - Axion:  implicit differentiation (1 backward solve per step)
  - XPBD:   backpropagation through time on Warp tape (T unrolled steps)
"""
import argparse
import json
import os
...
from axion import XPBDEngineConfig
...
class SplineAdam:
    ...
    def __init__(
        self, K: int, num_dofs: int, lr: float, betas=(0.9, 0.999), eps=1e-8, clip_grad=10.0
    ):
        self.lr = lr
...
class HelhestXPBDOptimizer(NewtonDifferentiableSimulator):
    def __init__(
        self,
        simulation_config: SimulationConfig,
        rendering_config: RenderingConfig,
        execution_config: ExecutionConfig,
        engine_config: XPBDEngineConfig,
    ...
    def compute_loss(self):
        T = self.clock.total_sim_steps
        weight = self.trajectory_weight / (T + 1)
        for t in range(T + 1):
            wp.launch(
                kernel=loss_kernel,
                dim=1,
    ...
    def train(self, iterations=50):
        ...
        self.spline_adam = SplineAdam(
            K=self.K, num_dofs=NUM_WHEEL_DOFS, lr=0.15, betas=(0.5, 0.999), clip_grad=10.0
        )
        self._apply_params(self.spline_params)

        # --- Optimization ---
        print(
            f"\nOptimizing: T={T}, dt={self.clock.dt:.4f}, K={self.K}, lr=0.15 (SplineAdam, BPTT)"
        )
        results = {
            "simulator": "XPBD",
            "problem": "helhest",
...
def main():
    ...
    engine_config = XPBDEngineConfig(
        ...
        soft_contact_relaxation=0.8,
        joint_linear_relaxation=0.7,
        joint_angular_relaxation=0.4,
        rigid_contact_relaxation=0.7,
        joint_linear_compliance=1e-5,
        joint_angular_compliance=1e-5,
