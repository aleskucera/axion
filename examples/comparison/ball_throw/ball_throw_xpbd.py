"""Ball throw optimization using Newton XPBD solver (BPTT).

Comparable to ball_throw_axion.py. Optimizes initial ball velocity via
backpropagation through time on Warp's tape.

Uses identical DT=3e-2 / T=50 as ball_throw_axion.py — the only difference
is the gradient method: Axion uses implicit AD, XPBD uses BPTT.
"""
import argparse
import json
import os
...
class BallThrowXPBDOptimizer(NewtonDifferentiableSimulator):
    def __init__(
        self,
    ...
    def train(self, iterations=15):
        ...
        T = self.clock.total_sim_steps
        print(f"\nOptimizing: T={T}, dt={self.clock.dt:.4f}, lr={LR} (XPBD, BPTT)")
        results = {
            "simulator": "XPBD",
            "problem": "ball_throw",
            "dt": self.clock.dt,
