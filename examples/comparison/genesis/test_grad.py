...
def test_diff_contact():
    ...
    box1 = scene.add_entity(
        gs.morphs.Box(
            size=box_size * vec_one, pos=box_pos_offset + 0.8 * box_spacing * np.array([0, 0, 1])
        ),
    )
    scene.build()
    ...
    box1.set_quat(
        R_to_quat(gs.euler_to_R([np.deg2rad(x_ang), np.deg2rad(y_ang), np.deg2rad(z_ang)]))
    )
...
def test_diff_solver(monkeypatch):
    ...
    for dL_dx, x_type in (
        ...
        for _ in range(TRIALS):
            ...
            rand_dx = rand_dx / max(
                np.linalg.norm(rand_dx, axis=0 if x_type in ("force", "aref", "efc_D") else (0, 1)),
                gs.EPS,
            )
            if x_type == "mass":
...
def test_differentiable_rigid(show_viewer):
    ...
    scene = gs.Scene(
        sim_options=gs.options.SimOptions(
            dt=dt, substeps=substeps, requires_grad=True, gravity=(0, 0, -1)
        ),
        rigid_options=gs.options.RigidOptions(
