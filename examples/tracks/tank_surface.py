...
class TankSurfaceSimulator(AbstractSimulator):
    ...
    def build_model(self) -> newton.Model:
        ...
        # Start position of Tank (Lifted higher to be safe above surface)
        start_z = 3.5
        start_xform = wp.transform(wp.vec3(0.0, 0.0, start_z), wp.quat_identity())

        # --- 3. Dynamic Base (Chassis) ---
        # Box chassis
        chassis_hx, chassis_hy, chassis_hz = 1.4, 1.0, 0.4
...
def tank_example(cfg: DictConfig):
    ...
    simulator = TankSurfaceSimulator(sim_config, render_config, exec_config, engine_config)
    simulator.run()
