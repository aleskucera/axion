...
@wp.kernel
def update_kernel(
    trajectory_qd_grad: wp.array(dtype=wp.spatial_vector, ndim=3),
    alpha: float,
    trajectory_qd: wp.array(dtype=wp.spatial_vector, ndim=3),
):
    tid = wp.tid()
    if tid > 0:
        return
    ...
    qd_y = trajectory_qd[0, 0, 0][1]
    qd_y_grad = trajectory_qd_grad[0, 0, 0][1]
    qd_y_new = qd_y - alpha * qd_y_grad
    trajectory_qd[0, 0, 0] = wp.spatial_vector(0.0, qd_y_new, 0.0, 0.0, 0.0, 0.0)
