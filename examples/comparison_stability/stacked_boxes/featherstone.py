"""Stacked-boxes stability benchmark — Newton Featherstone solver (penalty contacts).
...
Scene: 3-box inverted pyramid matching examples/box_stack.py.
  ...
  Mass ratio top/bottom ≈ 512:1.  Contacts: ke=1e7, kd=2e5, mu=0.1.
...
Usage:
    python examples/comparison/stacked_boxes/featherstone.py \
        --save examples/comparison/stacked_boxes/results/featherstone.json
"""
import argparse
import json
import os
...
# Local overrides for featherstone (much narrower search range)
BSEARCH_TOL = 1e-5
BSEARCH_MAX = 5e-4
...
def find_threshold(engine_config) -> dict:
    ...
    # Verify the assumed lower bound is actually stable before doubling from it.
    print(f"  probe dt={lo:.4f}s ...", end=" ", flush=True)
    n_evals += 1
    r = run_one(lo, engine_config)
    if not r["stable"]:
        print("UNSTABLE")
        print(f"  → max stable dt < {lo:.4f}s  (n_evals={n_evals})")
        return {"max_stable_dt": 0.0, "n_evals": n_evals, "hit_max": False}
    print("stable")
...
def run_one(dt: float, engine_config: FeatherstoneEngineConfig) -> dict:
...
def main():
    ...
    print("Featherstone — binary_search:")
