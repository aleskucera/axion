"""Stacked-boxes stability benchmark — Axion (Newton/Warp, forward only).
...
Scene: 3-box inverted pyramid matching examples/box_stack.py.
  ...
  Mass ratio top/bottom ≈ 512:1.  Stiff contacts (ke=1e7).
...
Usage:
    python examples/comparison/stacked_boxes/axion.py \
        --save examples/comparison/stacked_boxes/results/axion.json
"""
import argparse
import json
import os
...
from config import HX1
from config import HX2
from config import HX3
from config import KD
from config import KE
from config import KF
from config import MU
from config import STABILITY_TOL
from config import Z1
from config import Z2
from config import Z3
...
class StackedBoxesSim(InteractiveSimulator):
    ...
    def run_headless(self) -> dict:
        """Step the simulation manually and record top-box height at each step."""
        ...
        for step in tqdm(range(T)):
            ...
            # body_q: wp.array(dtype=wp.transform, shape=[num_bodies])
            # numpy layout per transform: [px, py, pz, qx, qy, qz, qw]
            bq = self.current_state.body_q.numpy()
            h = float(bq[2, 2])  # body 2 (top/large box), Z component
            ...
            if not np.isfinite(h):
                ...
                times.append(t)
                heights.append(None)
                break  # NaN/Inf: further steps are meaningless

            if abs(h - Z3) > STABILITY_TOL:
                stable = False  # mark but keep simulating to capture divergence
...
def run_one(dt: float, engine_config: AxionEngineConfig) -> dict:
