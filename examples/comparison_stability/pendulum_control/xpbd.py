"""Control stability benchmark — Newton XPBD solver (position-based dynamics).

Scene: single pendulum, pivot fixed at (0,0,2), 1m link, 1 kg.
       Start at Q_INIT=0 rad (hanging). Target: Q_TARGET=π/3 rad.

Uses the same implicit servo (JointMode.TARGET_POSITION) as Axion but driven
by the XPBD position-based solver instead of the Newton implicit solver.
...
Usage:
    python examples/comparison/control_stability/xpbd.py \
        --experiment dt_sweep \
        --save examples/comparison/control_stability/results/xpbd_dt.json
"""
import argparse
import json
import math
...
def _make_engine_config() -> XPBDEngineConfig:
    return XPBDEngineConfig(
        iterations=20,
...
def main():
    ...
    if args.experiment == "dt_sweep":
        ...
        print(f"XPBD — dt_sweep (kp={kp}, kd={kd}):")
        for dt in DT_VALUES:
        ...
        dt = GAIN_SWEEP_DT
        print(f"XPBD — gain_sweep (dt={dt}s):")
        for kp in KP_VALUES:
            kd = kd_from_kp(kp)
        ...
        print(f"XPBD — binary_search (kp={kp}, kd={kd}):")
