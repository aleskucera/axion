...
def test_revolute_velocity_control():
    ...
    # For velocity mode, 'ke' acts as velocity gain
    wp.copy(model.joint_target_ke, wp.array(np.array([1000.0], dtype=np.float32), dtype=wp.float32, device=model.device))
    ...
    # Setup Control
    target_vel = 1.0 # 1 rad/s
    
    print(f"Target Velocity: {target_vel:.4f}")
    ...
    assert final_error < 0.01, f"Velocity control failed to reach target. Error: {final_error}"

if __name__ == "__main__":
    test_revolute_velocity_control()
