...
- Target construction happens in `VanillaTrainer.preprocess_data_batch()` via
  `utils_provider.convert_next_states_to_prediction(states, next_states, dt)`.
- The relative/absolute behavior is controlled by `env.utils_provider_cfg.prediction_type`.
...
- `algorithm.sample_sequence_length` aligned with `env.utils_provider_cfg.num_states_history`
...
- `states_embedding` (typically identical to `states` for the current utils provider)
...
### Utils provider input processing

The `NeuralModelUtilsProvider` (`newton_based_integrator_neural.py`) ensures:
...
The trainer calls `utils_provider.process_neural_model_inputs(data)` before running the model forward pass.
...
Contacts are only used when corresponding tensors exist in the dataset and are listed in `inputs.low_dim`.
...
- **Config fields from the Warp pipeline may be misleading in the Axion/Newton pipeline**:
  - Fields like `states_frame` / `anchor_frame_step` exist in configs but are not used by the current utils provider (no frame conversion there).
