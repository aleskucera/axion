...
from axion.neural_solver.algorithms.mtl_trainer import MTLTrainer
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Train transformer-based multi-task network from YAML config."
    )
...
if __name__ == "__main__":
    ...
    if not args.train:
        raise NotImplementedError("--test is not implemented yet for train_mtl.py.")
    ...
    cfg["network"]["model_impl"] = "mtl"
    loss_cfg = cfg.get("algorithm", {}).get("loss", {}) or {}
    if "classification_num_classes" in loss_cfg:
        cfg["network"]["classification_num_classes"] = loss_cfg["classification_num_classes"]
