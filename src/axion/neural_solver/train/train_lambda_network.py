...
from axion.neural_solver.algorithms.lambda_classification_trainer import (
    LambdaClassificationTrainer,
)
...
def _parse_args():
    p = argparse.ArgumentParser(
        description="Train transformer-based per-step lambda classifier from YAML config."
    )
...
if __name__ == "__main__":
    ...
    if not args.train:
        raise NotImplementedError(
            "--test is not implemented yet for train_lambda_network.py."
        )
    ...
    algo = LambdaClassificationTrainer(
    ...
    print("Begin lambda classification training")
    algo.train()
