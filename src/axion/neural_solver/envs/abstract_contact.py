...
class AbstractContact:
    ...
    """
    def __init__(self, num_contacts_per_env, num_envs, model: newton.Model, device = 'cuda:0'):
        self.num_contacts_per_env = num_contacts_per_env
        self.num_envs = num_envs
        self.num_total_contacts = num_contacts_per_env * num_envs

        # essential variables
        self.contact_shape0 = torch.zeros(
            (self.num_total_contacts,),
            dtype = torch.int32, 
            device = device
        )
        self.contact_shape1 = torch.zeros(
            (self.num_total_contacts,), 
            dtype = torch.int32, 
            device = device
        )
        self.contact_point0 = torch.zeros(
            (self.num_total_contacts, 3),
            dtype = torch.float32,
            device = device
        )
        self.contact_point1 = torch.zeros(
            (self.num_total_contacts, 3),
            dtype = torch.float32,
            device = device
        )
        ...
        self.contact_offset0 = torch.zeros(
            (self.num_total_contacts, 3),
            dtype = torch.float32,
            device = device
        )
        self.contact_offset1 = torch.zeros(
            (self.num_total_contacts, 3),
            dtype = torch.float32,
            device = device
        )
        #  derived variables (not used by warp default integrators)
        self.contact_depth = torch.zeros(
            (self.num_total_contacts,),
            dtype = torch.float32,
            device = device
        )
        self.contact_thickness = torch.zeros(
            (self.num_total_contacts,),
            dtype = torch.float32,
            device = device
        )
    ...
    def create_warp_views(self, model: newton.Model):
        ...
        #Newton Contacts dont have depth and have thickness0/1 
