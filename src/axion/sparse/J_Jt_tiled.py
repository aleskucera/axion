...
@wp.kernel
def kernel_J_Jt_matvec_tiled(
    x: wp.array(dtype=wp.float32),  # (num_constraints)
    J_values: wp.array(dtype=wp.float32),  # (num_constraints)
    constraint_body_idx: wp.array(dtype=wp.int32),  # (num_constraints)
    out: wp.array(dtype=wp.float32),  # (constraint_idx)
):
    body_idx = wp.tid()
    ...
    y = wp.tile_sum(mul_tile)
    # print(y)

    # out = J * y

    y_broadcasted = wp.tile_broadcast(y, shape=J_body_tile.shape)
    out_tile = wp.tile_map(wp.mul, J_body_tile, y_broadcasted)
...
def main():
    ...
    tmp_scatter = wp.zeros(NUM_BODIES)
    ...
    print(f"{out_scatter_host}")
    print(f"{out_tiled_host}")
