...
def measure_matvec_time(op, num_repeats=50):
    x = wp.ones(op.shape[1], dtype=wp.float32)
    y = wp.zeros(op.shape[0], dtype=wp.float32)
    z = wp.zeros(op.shape[0], dtype=wp.float32)
...
def main():
    ...
    print(
        f"Total rows: {op.shape[0]}, total cols: {op.shape[1]}\n"
