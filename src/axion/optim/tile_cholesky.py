    ...
    def blocked_cholesky_kernel(
        ...
        for k in range(0, n, block_size):
            ...
            A_kk_tile = wp.tile_load(
                A, shape=(block_size, block_size), offset=(k, k), storage="shared"
            )
            ...
            if k + block_size > active_matrix_size:
                ...
                num_iterations = (
                    num_tile_elements + num_threads_per_block - 1
            ...
            for i in range(end, n, block_size):
                A_ik_tile = wp.tile_load(
                    A, shape=(block_size, block_size), offset=(i, k), storage="shared"
                )
                # The following if pads the matrix if it is not divisible by block_size
...
class BlockCholeskySolver:
    ...
    def __init__(self, max_num_equations: int, block_size=16, device="cuda"):
        ...
        self.L = wp.zeros(
            shape=(self.max_num_equations, self.max_num_equations), dtype=float, device=self.device
        )
        self.y = wp.zeros(
            shape=(self.max_num_equations, 1), dtype=float, device=self.device
        )  # temp memory
    ...
    def factorize(self, A: wp.array(dtype=float, ndim=2), num_active_equations: int):
        ...
        assert (
            num_active_equations <= self.max_num_equations
        ), f"Number of active equations ({num_active_equations}) exceeds maximum allowed ({self.max_num_equations})"

        padded_n = (
            (num_active_equations + self.block_size - 1) // self.block_size
        ) * self.block_size
        ...
        assert (
            A.shape[0] >= padded_n
        ), f"Matrix A must be at least {padded_n}x{padded_n} to accommodate padding"

        self.active_matrix_size.zero_()
        wp.copy(
            self.active_matrix_size, wp.array([num_active_equations], dtype=int, device=self.device)
        )
    ...
    def factorize_dynamic(
        self, A: wp.array(dtype=float, ndim=2), num_active_equations: wp.array(dtype=int, ndim=1)
    ):
        """
        Computes the Cholesky factorization of a symmetric positive definite matrix A in blocks.
        It returns a lower-triangular matrix L such that A = L L^T.
    ...
    def solve(self, rhs: wp.array(dtype=float, ndim=2), result: wp.array(dtype=float, ndim=2)):
        ...
        if self.active_matrix_size_int > 0:
            ...
            assert (
                rhs.shape[0] >= padded_n
            ), f"Matrix b must be at least {padded_n}x1 to accommodate padding"

            assert result.shape[1] == 1, "Matrix result must be a column vector"
            assert (
                result.shape[0] >= padded_n
            ), f"Matrix result must be at least {padded_n}x1 to accommodate padding"
...
class CholeskySolverNumPy:
    ...
    def factorize(self, A: np.ndarray, num_active_equations: int):
        ...
        assert A.shape[0] == A.shape[1], "Matrix A must be square"
        assert (
            A.shape[0] >= num_active_equations
    ...
    def solve(self, rhs: np.ndarray, result: np.ndarray):
        ...
        """
        assert (
            self.num_active_equations <= self.max_num_equations
...
def test_cholesky_solver(
    n, warp_solver: BlockCholeskySolver, headless: bool = False, device: str = "cuda"
):
