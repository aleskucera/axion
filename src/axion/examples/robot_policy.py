...
ROBOT_CONFIGS = {
    ...
    "g1_23dof": RobotConfig(
        asset_dir="unitree_g1",
        policy_path={
            "mjw": "rl_policies/mjw_g1_23DOF.pt",
            "physx": "rl_policies/physx_g1_23DOF.pt",
        },
...
def quat_rotate_inverse(q: torch.Tensor, v: torch.Tensor) -> torch.Tensor:
    ...
    if q_vec.dim() == 2:
        c = (
            q_vec
            * torch.bmm(q_vec.view(q.shape[0], 1, 3), v.view(q.shape[0], 3, 1)).squeeze(-1)
            * 2.0
        )
    else:
...
def load_policy_and_setup_tensors(
    example: Any, policy_path: str, num_dofs: int, joint_pos_slice: slice
):
    ...
    example.joint_pos_initial = torch.tensor(
        joint_q[joint_pos_slice], device=device, dtype=torch.float32
    ).unsqueeze(0)
...
class Example:
    def __init__(
        ...
        self.physx_to_mjc_indices = torch.tensor(
            physx_to_mjc, device=self.torch_device, dtype=torch.long
        )
        self.mjc_to_physx_indices = torch.tensor(
            mjc_to_physx, device=self.torch_device, dtype=torch.long
        )
    ...
    def capture(self):
        ...
        if wp.get_device().is_cuda and wp.is_mempool_enabled(wp.get_device()):
            ...
            torch_tensor = torch.zeros(
                self.config["num_dofs"] + 6, device=self.torch_device, dtype=torch.float32
            )
            self.control.joint_target = wp.from_torch(
                torch_tensor, dtype=wp.float32, requires_grad=False
            )
    ...
    def step(self):
        ...
        with torch.no_grad():
            ...
            a_with_zeros = torch.cat(
                [torch.zeros(6, device=self.torch_device, dtype=torch.float32), a.squeeze(0)]
            )
