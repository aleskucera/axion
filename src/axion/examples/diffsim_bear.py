...
DEFAULT_BEAR_PATH = os.path.join(
    newton.examples.get_asset_directory(), "bear.usd"
)  # Path to input bear asset
...
@wp.kernel
def network(
    phases: wp.array2d(dtype=float),
    weights: wp.array2d(dtype=float),
    tet_activations: wp.array2d(dtype=float),
):
    # output tile index
    i = wp.tid()
...
class Example:
    def __init__(self, viewer, verbose=False, sim_steps=300):
        ...
        for _i in range(self.sim_steps):
            self.tet_activations.append(
                wp.zeros(self.model.tet_count, dtype=float, requires_grad=True)
            )
    ...
    def forward(self, frame):
        ...
        wp.launch(
            kernel=compute_phases, dim=self.phase_count, inputs=[self.phases[frame], self.sim_time]
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Number of simulation steps to execute in a training run.",
    )
