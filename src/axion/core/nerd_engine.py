...
class NerdEngine(SolverBase):
    ...
    def __init__(self, 
        ...
        #########################################
        #  Classic AxionEngine initialization
        #########################################
        ...
        loaded_nn_model, robot_name = torch.load(nn_model_path, map_location= self.device, weights_only= False)
        ...
        gravity_vector = torch.zeros((self.num_models, 3), device= self.device)
        gravity_vector[:, 1] = -1.0  # Gravity in negative Y direction
    ...
    def step(
        ...
        joint_acts = torch.zeros((self.num_models, 1), device= self.device)

        # Preprocess contacts
        max_num_contacts_per_model = 4
        contact_normals = wp.to_torch(contacts.rigid_contact_normal).clone()
        contact_depths = -100* torch.zeros((self.num_models, max_num_contacts_per_model)) # FIX: wp.to_torch(contacts.rigid_contact_d)
        contact_thickness = wp.to_torch(contacts.rigid_contact_thickness0).clone()  # FIX: pick 0 or 1?
        contact_points_0 =  wp.to_torch(contacts.rigid_contact_point1).clone()
        contact_points_1 = wp.to_torch(contacts.rigid_contact_point1).clone()
        contacts = {
        ...
        state_out = state_in
        state_out.joint_q = wp.from_torch(state_predicted[:2])
        state_out.joint_qd = wp.from_torch(state_predicted[2:])