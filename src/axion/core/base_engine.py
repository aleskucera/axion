...
@wp.kernel
def _eisenstat_walker_kernel(
    res_norm_sq: wp.array(dtype=float),
    prev_res_norm_sq: wp.array(dtype=float),
    prev_eta: wp.array(dtype=float),
    gamma: float,
    alpha: float,
    eta_min: float,
    eta_max: float,
    # Output: per-world forcing term η_k = relative tolerance for PCR.
    eta_k: wp.array(dtype=float),
):
    """Eisenstat-Walker choice 2 with the PETSc-style safeguard.

    For each world independently:

        ratio = sqrt(res_norm_sq / prev_res_norm_sq)        (= ||r_k|| / ||r_{k-1}||)
        eta_k = gamma * ratio^alpha
        # Safeguard: prevent η from shrinking faster than the
        # quadratic-residual rate would justify, which would ask PCR
        # for unrealistic precision after one good outer step.
        guard = gamma * prev_eta^alpha
        if guard > 0.1:
            eta_k = max(eta_k, guard)
        eta_k = clip(eta_k, eta_min, eta_max)

    All math in float32 — eta is unitless and bounded in [eta_min, eta_max]
    so precision is fine. If prev_res_norm_sq is 0 (first NR iter, before
    any prior residual exists), we return eta_max (loose) — but the caller
    should not invoke this kernel before iter `warmup_iters`, so this is
    just a safety fallback.
    """
    tid = wp.tid()
    if tid >= res_norm_sq.shape[0]:
        return

    prev = prev_res_norm_sq[tid]
    if prev <= 0.0:
        eta_k[tid] = eta_max
        return

    # Ratio of NR residual norms (not norms-squared) — sqrt the inputs.
    # Clamp the radicand to 0 in case of numerical noise pushing it
    # slightly negative.
    curr = res_norm_sq[tid]
    if curr < 0.0:
        curr = float(0.0)
    ratio = wp.sqrt(curr / prev)

    eta = gamma * wp.pow(ratio, alpha)

    # PETSc-style safeguard: don't shrink eta past gamma*prev_eta^alpha
    # when that quantity is non-trivial. Prevents one outlier "great"
    # iter from over-tightening subsequent iters.
    guard = gamma * wp.pow(prev_eta[tid], alpha)
    if guard > 0.1:
        eta = wp.max(eta, guard)
