...
NN_BASE_PATH = Path.cwd() /"src"/"axion"/"neural_solver"/"train"/"trained_models"/"sweep1"/"h1a1hko8"
...
class AxionEngineWithNeuralLambdas(AxionEngineBase):
    ...
    def step(
        ...
        if self.neural_dataset_logger is not None:
            ...
            body_q_2d = state_in.body_q.reshape((self.nn_predictor.num_worlds, self.nn_predictor.bodies_per_world))
            ...
            self.neural_dataset_logger.append_step(
                ...
                predicted_next_lambdas=self._to_numpy(predicted_next_lambdas),
